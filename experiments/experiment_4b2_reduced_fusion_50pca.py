"""
EXPERIMENT 4B2: REDUCED FEATURE FUSION
======================================

12 JIT features + first 50 PCA-reduced CodeBERT components

Methodology:
- Project-wise chronological 70/15/15 split
- PCA was already fitted on TRAIN ONLY
- Uses first 50 components from existing 384-D PCA representation
...
Output:
    results/experiment_4b2_reduced_fusion_50pca/
"""
...
OUTPUT_DIR = (
    BASE_DIR
    / "results"
    / "experiment_4b2_reduced_fusion_50pca"
)
...
def create_reduced_fused_split(
    ...
    # --------------------------------------------------------
    # IMPORTANT:
    # Select FIRST 50 components
    #
    # These are components 1–50 from the SAME PCA that was
...
def save_confusion_matrix_plot(
    ...
    ax.set_title(
        f"4B2 Reduced Fusion - {model_name}"
    )
...
def save_roc_curve(
    ...
    plt.title(
        f"4B2 Reduced Fusion ROC Curve - {model_name}"
    )
...
def save_pr_curve(
    ...
    plt.title(
        f"4B2 Reduced Fusion PR Curve - {model_name}"
    )
...
def main():
    ...
    logging.info(
        "  CodeBERT features: FIRST 50 of existing 384 PCA components"
    )
    logging.info(
        "  Total fused features: 62"
    )
    logging.info(
    ...
    assert fused["train"].shape[1] == 62
    assert fused["validation"].shape[1] == 62
    assert fused["test"].shape[1] == 62
    ...
    config = {
        ...
        "pca_selection": "first_50_components_of_existing_384d_pca",
        "join_key": "commit_id",
        "resampling": False,
        "random_split": False,
        "threshold": 0.5,
    ...
    for model_name, model in models.items():
        ...
        model_path = (
            MODELS_DIR
            / f"{model_name}_jit_codebert_50pca.joblib"
        )
    ...
    # ========================================================
    # COMPARISON: 4A vs 4B1 vs 4B2
    # ========================================================

    logging.info("\n" + "=" * 70)
    logging.info("COMPARISON: 4A vs 4B1 vs 4B2")
    logging.info("=" * 70)

    # Existing completed test results.
    # These are used ONLY for post-experiment comparison;
    ...
    # Convert current 4B2 test results to the same schema.
    current_results = []
    ...
    # Display in a natural experiment/model order.
    variant_order = {"4A": 0, "4B1": 1, "4B2": 2}
    model_order = {"Random Forest": 0, "XGBoost": 1}

    comparison_df["_variant_order"] = comparison_df["variant"].map(
        variant_order
    )
    comparison_df["_model_order"] = comparison_df["model"].map(
        model_order
    )

    comparison_df = comparison_df.sort_values(
        ["_variant_order", "_model_order"]
    ).drop(
        columns=["_variant_order", "_model_order"]
    ).reset_index(drop=True)

    comparison_path = (
        METRICS_DIR
        / "experiment_4a_4b1_4b2_comparison.csv"
    )
    ...
    # --------------------------------------------------------
    # 4B2 improvement over 4A and 4B1
    # --------------------------------------------------------

    improvement_rows = []

    for model_label in ["Random Forest", "XGBoost"]:
        current = current_results_df[
            current_results_df["model"] == model_label
        ].iloc[0]

        for baseline_variant in ["4A", "4B1"]:
            baseline = historical_results[
                (historical_results["variant"] == baseline_variant)
                & (historical_results["model"] == model_label)
            ].iloc[0]

            improvement_rows.append({
                "model": model_label,
                "comparison": f"4B2_vs_{baseline_variant}",
                "delta_f1": current["f1"] - baseline["f1"],
                "delta_mcc": current["mcc"] - baseline["mcc"],
                "delta_roc_auc": current["roc_auc"] - baseline["roc_auc"],
                "delta_pr_auc": current["pr_auc"] - baseline["pr_auc"],
            })
    ...
    improvement_path = (
        METRICS_DIR
        / "experiment_4b2_improvement_vs_4a_4b1.csv"
    )
